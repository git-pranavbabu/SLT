...
OUTPUT_DIR = 'MP_Data_Pro'
TARGET_GLOSSES = ['hello', 'drink']
SEQUENCE_LENGTH = 30
NUM_WORKERS = 4

# --- CONSTANTS ---
# We enforce these shapes for the LSTM. 
# If MediaPipe gives less, we pad. If more, we slice (or error).
...
def init_worker():
    ...
    try:
        base_pose = python.BaseOptions(model_asset_path=MODEL_PATHS['pose'])
        worker_models['pose'] = vision.PoseLandmarker.create_from_options(
            vision.PoseLandmarkerOptions(base_options=base_pose))
        
        base_hand = python.BaseOptions(model_asset_path=MODEL_PATHS['hand'])
        ...
        base_face = python.BaseOptions(model_asset_path=MODEL_PATHS['face'])
...
def process_single_video(task_data):
    ...
    try:
        ...
        # Validation inside worker
        # We accept -1 or 0 or anything negative as "Go to the end"
        ...
        target_indices = np.linspace(start_frame, end_frame, SEQUENCE_LENGTH, dtype=int)
        target_set = set(target_indices)
        
        final_sequence = []
        
        # Fast Seek Replacement: Robust Sequential Read
        # cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame) -> often fails on these MKV/MP4 files
        # Instead, we just burn through frames until we hit start_frame
        current_frame = 0
        ...
        # We read sequentially from start_frame to end_frame
        # This is more I/O efficient than seeking 30 times
        while current_frame <= end_frame:
            ...
            if current_frame in target_set:
                ...
                if face_res.face_landmarks:
                    ...
                    # Logic: If model outputs 468, we pad. If 478, we use.
                    count = len(landmarks)
                ...
                if hand_res.hand_landmarks:
                    for i, handedness in enumerate(hand_res.handedness):
                        # Guard: Handedness might be missing/empty
                ...
                # Concatenate
                frame_vec = np.concatenate([pose_flat, face_flat, lh_flat, rh_flat])
                final_sequence.append(frame_vec)
                
                if len(final_sequence) >= SEQUENCE_LENGTH:
                    break
        ...
        # Final Check & Padding
        # Sometimes we are off by 1-2 frames due to EOF or seeking quirks
        if len(final_sequence) < SEQUENCE_LENGTH:
            if len(final_sequence) >= SEQUENCE_LENGTH - 5 and len(final_sequence) > 0:
                logging.info(f"Padding {video_id} from {len(final_sequence)} to {SEQUENCE_LENGTH}")
                while len(final_sequence) < SEQUENCE_LENGTH:
                    final_sequence.append(final_sequence[-1])
            else:
                logging.warning(f"Short sequence for {video_id}: {len(final_sequence)}/{SEQUENCE_LENGTH}")
                return None
...
# --- MAIN ---
def main():
    print("--- WLASL GOLD PROCESSOR (PATCHED) ---")
    
    # Pre-Flight Check
    missing_models = [p for p in MODEL_PATHS.values() if not os.path.exists(p)]
    if missing_models:
        print(f"CRITICAL: Missing models: {missing_models}")
        return
    ...
    for entry in content:
        gloss = entry['gloss']
        if gloss not in TARGET_GLOSSES: continue
        ...
        for inst in entry['instances']:
            ...
            # Locate Source
            vid_path = os.path.join(VIDEO_DIR, f"{yt_id}.mp4")
            if not os.path.exists(vid_path):
                vid_path = os.path.join(VIDEO_DIR, f"{yt_id}.mkv")
            ...
            # Only subtract 1 if it is a specific frame number. 
            # If it is -1, keep it -1 to signal "End of Video" to the worker.
            if f_end > 0:
                f_end -= 1
